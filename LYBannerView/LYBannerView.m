...
static const NSInteger kTime = 5;
static const CGFloat kHorMargin = 10;
static const CGFloat kVerMargin = 5;
static const CGFloat kDescLabelHeight = 20;
@interface LYBannerView ()<UIScrollViewDelegate>
//轮播的图片数组
...
#pragma mark - Init
//创建用来缓存图片的文件夹,initialize只会创建一次
+ (void)initialize {
    NSString *cache = [[NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES) lastObject] stringByAppendingPathComponent:@"LYBannerImage"];
    BOOL isDir = NO;
...
+ (instancetype)bannerViewWithImageArray:(NSArray *)imageArray describeArray:(NSArray *)describeArray {
    LYBannerView *bannerView = [self new];
...
- (void)imageClick {
    ...
    } else if ([_delegate respondsToSelector:@selector(bannerView:clickImageAtIndex:)]){
        [self.delegate bannerView:self clickImageAtIndex:self.currIndex];
    }
}

- (void)dwonloadImages:(int)index {
    NSString *key = self.imageArray[index];
    //从沙盒中取图片
    NSString *path = [[[NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES) lastObject] stringByAppendingPathComponent:@"LYBannerImage"] stringByAppendingPathComponent:[key lastPathComponent]];
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    if (offsetX < self.width * 2) {//right
        if (self.changeMode == ChangeModeFade) {
...
- (void)layoutSubviews {
    ...
    _describeLabel.frame = CGRectMake(0, self.height - kDescLabelHeight, self.width, kDescLabelHeight);
    //重新计算pageControl的位置
...
- (void)startTimer {
    ...
    self.timer = [NSTimer timerWithTimeInterval:_time < 2? kTime: _time target:self selector:@selector(nextPage) userInfo:nil repeats:YES];
...
- (void)setImageArray:(NSArray *)imageArray {
    ...
    for (int i = 0; i < imageArray.count; i++) {
        if ([imageArray[i] isKindOfClass:[UIImage class]]) {
            ...
            [self dwonloadImages:i];
        }
    }
    
    //防止在滚动过程中重新给imageArray复制时报错
    if (_currIndex >= _images.count) {
        _currIndex = _images.count - 1;
    }
    _currImageView.image = _images[_currIndex];
...
- (void)setPagePosition:(PageControlPosition)pagePosition {
    ...
    CGFloat centerY = self.height - size.height * 0.5 - kVerMargin - (_describeLabel.hidden?0: kDescLabelHeight);
    CGFloat pointY = self.height - size.height - kVerMargin - (_describeLabel.hidden?0: kDescLabelHeight);
    if (_pagePosition == PositionNone || _pagePosition == PositionBottomCenter)
        ...
        _pageControl.center = CGPointMake(self.width * 0.5, size.height * 0.5 + kVerMargin);
    else if (_pagePosition == PositionBottomLeft)
        _pageControl.frame = CGRectMake(kHorMargin, pointY, size.width, size.height);
    else
        _pageControl.frame = CGRectMake(self.width - kHorMargin - size.width, pointY, size.width, size.height);
}
